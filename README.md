...
In this project, I worked with an eCommerce application. Through this project I show some Security & DevOps skills learnt. This involves adding proper authentication and authorization controls so users can only access their data, and that data can only be accessed in a secure way.

## Project Description
The project is written in Java using Spring Boot, Hibernate ORM, and the H2 database. H2 is an in memory database, so if you need to retry something, every application startup is a fresh copy.
...
## Testing
Will implement unit tests demonstrating at least 80% code coverage.
